class TrialDispatcher
  def self.call(entry:, now: Time.current)
    ...
    run = Distributor.with_reservation_lock do
      ...
      next unless %w[ARCHIVED TRIAL].include?(entry.level)
      active ||= entry.trial_runs.create!(status: 'pending', explanation: { 'requested_at' => now.iso8601, 'minimum_groups' => AppConfig.trial.group_min })
      active.lock!
      next active if active.status == 'running'
      entry.update!(level: 'TRIAL') unless entry.level == 'TRIAL'
      ...
      needed = [AppConfig.trial.group_min - sent_count - still_pending, 0].max
      if groups.length < needed
        active.update!(explanation: active.explanation.merge('reason' => 'waiting_for_groups', 'eligible_groups' => groups.length,
          'sent_groups' => sent_count, 'reserved_groups' => still_pending, 'minimum_groups' => AppConfig.trial.group_min,
      ...
      if queued.length < needed
        queued.each(&:destroy!)
        queued.clear
        active.update!(explanation: active.explanation.merge('reason' => 'waiting_for_quota', 'minimum_groups' => AppConfig.trial.group_min, 'last_checked_at' => now.iso8601))
        next active
      end
      ...
      active.update!(status: 'running', started_at: active.started_at || now, ends_at: now + AppConfig.trial.duration,
        explanation: active.explanation.merge('reason' => 'groups_reserved', 'minimum_groups' => AppConfig.trial.group_min,
