...
一个以 QQ 群自然聊天为入口的互联网文化档案馆。群友正常发图、转发和回复，Rails 在后台观察、建档、试吃、传播并记录自然复活。**产品不使用 AI，也不分析“笑死 / 典”等文本语义。**

这是一个完整 Rails 应用：PostgreSQL、持久任务队列、对象存储、公开网站、账号、群认领和馆务流程共用一份领域数据。默认接入两个 Fake NapCat 机器人；不会登录真实 QQ 或向真实群发消息。
...
首页有新鲜出土、试吃、试吃结果、传播榜、自然搬运榜、复活、经典、今日考古、群馆和搬运者。目录支持 S-ID / 标题 / 描述 / 公开群名 / 公开搬运者 / 标签搜索，按等级、群、安全标签和日期过滤、分页。
...
普通用户在群页浏览档案。已认领管理员才看到“管理本群”，可改胃口、模式、试吃、冷却、每日摄入量和隐私。馆务入口只对 `curator` 开放，用于安全审核、举报处理、打码替换、资源下架、候选隔离和可撤销合并；没有公开的 SaaS 后台侧栏。
...
3. 结束候选观察。符合规则后获得永久 `S-xxxxxx`，仍处于保守的 RED / 隐藏状态。
4. 审核项目原创安全示例，进入 GREEN；寻找试吃席位。
...
**采集：** 默认观察 30 分钟，至少 2 名独立非作者参与、得分至少 5。每人只计最强信号：回复 2、引用 2.5、Reaction 1.5、再次自然发送 5；合并转发基础分 1。作者自回、自赞、机器人回声和撤销互动不计。到期才建档；已知重复可直接记原编号下的新出现。超过补录窗口的过期消息不追认。
...
**试吃：** 默认 3–5 个群，OPT_IN 按最近参与时间轮换，不足时补 FALLBACK；OPT_OUT 永不入选。至少凑够最少席位才开始。固定 Reaction 可配置，并保留直接回复、引用和独立人数。正反馈群比例的分母是全部已送达试吃群；晋级另看有反馈群中的正向比例，避免把沉默当反对票。负反馈率只以明确的三种 Reaction 为分母。
...
新内容默认 `RED + hidden`，只建档、不自动传播。馆务可审核 GREEN 或带多标签的 YELLOW；只有接受全部 YELLOW 标签的群才能收到。RED 永不自动跨群传播，PRIVACY / EXTREME 是平台硬禁标签，群配置不能移除。文件 SHA 黑名单、来源黑名单、资源隐藏和传播暂停独立生效。
...
公共页面不输出外部 QQ 标识。群可以匿名、只公开统计、隐藏整个页面、隐藏成员；只公开统计的群不会出现在具体条目时间线和试吃席位关联中。未知搬运者默认匿名且不公开个人主页。公开页面中的所有内容仍以人工允许公开为前提。
...
先阅读 [NapCat / OneBot 能力核验](docs/napcat-capabilities.md)，其中保留官方文档与源代码链接、Reaction 扩展和消息 ID 的能力边界。

接口为 `Adapters::OneBotAdapter`，Fake 使用数据库里的群、角色、名片、内容和消息。已提供 `RealNapCatAdapter` HTTP 实现，支持群信息、成员角色、自身名片、获取消息 / 合并转发、图片与转发投放；**真实 QQ 连接未进行在线验收**。

接入时建立 `BotAccount`、`BotConnection(adapter: 'real', endpoint: ...)` 与群成员关系，开启 `NAPCAT_ENABLED` 并设置 API / Webhook token。配置 NapCat HTTP 事件上报到：

```text
POST /onebot/:connection_id/events
Authorization: Bearer <NAPCAT_WEBHOOK_TOKEN>
```

入口核对 `self_id` 与对应 BotAccount，并先持久化规范事件、排队处理。可选前向 WebSocket 入口为 `bin/onebot-ws CONNECTION_ID`，需配置 `NAPCAT_WS_URL`；使用同一入库层。HTTP actions 用于出站发送。多个连接可指定独立 endpoint 和 `credential_env_key`。通过 `NAPCAT_CONNECTION_TOKENS` JSON 将这个键映射到各自的 API token；未配置独立键时使用部署级 `NAPCAT_ACCESS_TOKEN`。原始 token 不存入 BotConnection 数据库记录。

OneBot 11 标准没有统一群 Reaction；NapCat 扩展需提供群、消息、操作人、emoji 与 add/remove。缺少身份或增删方向时记录为不支持 / 未知，不编造统计。默认 emoji ID 映射是配置示例，接入自己的 NapCat 版本时需核对。

事件 ID 和消息唯一键处理重复上报；乱序回复先暂存，再绑定目标消息；Reaction 有串行更新与顺序规则。QQ 时间戳只有秒级，无法恢复未提供的服务端全序；同秒事件按首次入库序号作确定性处理。断线期间 NapCat 并不保证补发全部历史，本项目不会声称能恢复没收到的事件。

发送先提交 `sending` 再调用 Adapter，成功记录消息回执；超时、缺失回执、含糊的 5xx 进入 `uncertain`，不盲目重发。必须先核验 QQ 实际消息，才能手动处理不确定投放。API `echo` 不是服务端幂等键。这是外部协议的交付边界，不用数据库事务假装 exactly-once。
...
本仓库包含 Git 历史、Dockerfile、Compose、migrations、fixtures、测试和 GitHub Actions（原生 PostgreSQL 16）。没有绑定任何 GitHub 远端；可自行设置 remote 后推送 `main`，无需重新生成项目。
